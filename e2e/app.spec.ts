 ...
 * UploadClient.reconcile). It is not running in the preview/e2e environment,
 * so chromium would spend hundreds of ms per probe waiting for the connection
 * to be refused — enough to push toast/view assertions past their default 5s
 * timeout on slower runs. Always abort these requests immediately.
 */
async function stubUploadService(page: Page): Promise<void> {
